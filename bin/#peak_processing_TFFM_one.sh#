...
if [ -z "$3" ]
    then
        echo "No PFM folder specified. Using: /storage/scratch/marius/GEO/src/PFM_JASPAR";
        pfm_folder="../data/pfm"
    else
        pfm_folder=$3
fi
...
if [ -z "$4" ]
    then
        echo "No TF to PFM mapping file specified. Using: /storage/scratch/marius/GEO/bin/GEO_TF_JASPAR_mapping_18_09_2017.tsv";
...
d=$folder_to_process
   ...
   out_dir=$result_folder/$(basename $in_dir)/${d%/*}
   ...
   ##### DEBUG
#   echo "Dataset from folder: $data_set"
#   echo "Cell line from folder: $cell_line"
#   echo "TF name from folder: $tf_name"
#   echo "-----------------------------------"
   ...
   if [ $data_type = "JAMM" ]
     ...
     echo "No supported data_type passed as argument. Only JAMM, MACS or OMalley supported.";
     exit 1
  fi
